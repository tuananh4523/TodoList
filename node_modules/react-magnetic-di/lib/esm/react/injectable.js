...
export function injectable(from, implementation, {
  ...
  if (typeof impl === 'function') {
    impl.displayName = displayName || getDisplayName(impl) || getDisplayName(from, 'di');
  ...
  if (diRegistry.has(impl) && diRegistry.get(impl).from !== from) {
    warnOnce(`You are trying to use replacement "${displayName || impl.displayName}" on multiple injectables. ` + `That will override only the last dependency, as each replacement is uniquely linked.`);
  }
  diRegistry.set(impl, {
