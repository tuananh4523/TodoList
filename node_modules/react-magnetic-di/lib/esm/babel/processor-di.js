...
function processReference(t, path, locationValue, state) {
  ...
  locationValue.dependencyRefs.forEach(n => {
    ...
    if (name === (self == null ? void 0 : self.name)) {
      shadowsOwnName = true;
      return;
    }
  ...
  // add di there
  const declaration = t.variableDeclaration('const', [t.variableDeclarator(t.arrayPattern(elements), t.callExpression(t.identifier(state.diIdentifier.name), [self && !shadowsOwnName ? t.identifier(self.name) : t.nullLiteral(), ...args]))]);
